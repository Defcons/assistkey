...
- **User-file paths MUST go through `paths.app_dir()` — never `Path(__file__)`.** In a frozen one-file build `__file__` resolves inside the ephemeral `_MEIPASS` extraction dir (wiped on exit), so `config.json`/`assistkey.log`/the autostart target written there vanish on every restart. `paths.app_dir()` returns the exe's folder when frozen, the source dir otherwise. `config.CONFIG_PATH`, `diag.LOG_PATH`, and `autostart._launch_command` all rely on it (autostart also registers the exe directly when frozen, not the VBS). Any new persistent file must use it too.
...
- **`_run_utterance` MUST always return so `_active`/`_idle` reset — its teardown can NOT be allowed to hang.** `start_utterance`'s `finally` is the only thing that clears `_active` and sets `_idle`; if `_run_utterance` hangs (it did: `finally: await forwarder` never returned when forward_audio's teardown stalled on a dead mic / half-open socket), the state stays stuck and EVERY later hotkey press dead-ends in `restart_utterance`'s idle-wait — a permanently dead hotkey. The `await forwarder` is therefore bounded (`asyncio.wait(..., timeout=3)`, then abandon). Keep any new awaits in that teardown bounded. A hold whose loudest processed sample stays below `SILENCE_PEAK` (mic off/muted — a headset that's OFF still delivers *silent* frames, so this is a LEVEL check, not a frame-count one) → a `__nomic__` error, not shipped to HA. See Journal 2026-08-27 mic-hang.
