...
- [ ] **[MEDIUM] TTS fetch is uninterruptible with a 15 s timeout** (`assist_client._play`: `urlopen(timeout=15)` in an executor). Root cause of TWO symptom classes: (a) a barge-in during a slow/hanging fetch does nothing for up to ~15 s (`sd.stop()` can't interrupt the fetch; the event loop is stuck at `await self._play`, so the queued `__cancel__` isn't processed), and (b) the reply popup can linger while the fetch drags. The 2026-08-27 fix made the STUCK-STATE safe (wake no longer gets stuck), but the responsiveness gap remains. Fix ideas: shorter timeout, or make the fetch interruptible (close the response from another thread on cancel), or check a cancel flag between fetch and `sd.play`.
