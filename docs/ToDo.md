...
- [ ] **[LOW] `_run_utterance` teardown stops sd stream on the loop thread** (`forward_audio` finally: `stream.stop(); stream.close()`). The 2026-08-27 mic-hang was an *await* hang (now bounded), and in that incident the loop stayed alive, so these sync sd calls did NOT block it. But on some devices a sd stop/close *could* block, which would freeze the whole asyncio loop (the bounded `await forwarder` can't help then). If a future hang shows the loop itself frozen (no `restart_utterance` logs during the stick), move the stream stop/close into `run_in_executor` with its own timeout. Not observed; deferred.
...
- [x] 2026-08-27 — **Settings dialog is now height-capped + scrollable.** Save/Cancel pinned in a bottom bar OUTSIDE a `CTkScrollableFrame`; the window sizes to content but caps to the monitor work area (then the body scrolls), so Save is always reachable on a 1080p screen. Fixed 416 px width (a scrollable frame doesn't report its content's width → `winfo_reqwidth` collapses and clips); the idle scrollbar is hidden when everything fits. Verified at simulated 1080p (caps to 984 px, Save mapped) + normal render (no scrollbar). See Journal 2026-08-27.
