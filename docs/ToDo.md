...
- [ ] **[MEDIUM, pre-public] Settings dialog too tall for 1080p screens.** It grew with each feature and is now ~1051 px; on a 1080p work area (~1032 px) the Save/Cancel buttons sit off-screen (fine on ≥1440p, e.g. the dev machine's 2560×1392). Make the settings body a `CTkScrollableFrame` (or otherwise height-capped) with the button bar pinned outside the scroll area, so Save is always reachable. Was already near the limit before the 2026-08-27 mic-boost rows; do before a public release. (Own risk/visual testing — kept separate from the mic feature.)
