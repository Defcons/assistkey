...
- **FACT** — Tray icon colour = state: red `DISCONNECTED_COL` (not connected), grey `IDLE_COL` (connected/ready), green `ACTIVE_COL` (listening/working). Driven by `("connected",)`/`("disconnected",)` from the client + `("listening",)`; `App._set_idle_icon` picks red vs grey from `self._connected`. Tray menu: Settings / Stop / Quit.
...
- **FACT** (best-effort, HA-version-sensitive) — Conversation continuity: the client captures `conversation_id` from pipeline events and replays it on the next run within `CONVERSATION_TTL` (60 s) so back-to-back commands share context. Follow-up (`config.follow_up_enabled`, default OFF): if `intent-end` carries `continue_conversation`, `consume_follow_up()` makes `app` auto-listen for the next turn (VAD-ended; wake stays paused across the chain) instead of dismissing. If HA's field path differs, follow-up simply never fires — no harm. Tests: `test_consume_follow_up_*`.
